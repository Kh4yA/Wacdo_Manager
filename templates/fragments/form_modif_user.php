...
<form action="/modifUser/<?= htmlspecialchars($userCurrent->getId()) ?>" method="POST" class="flex item-center space-between">
    ...
    <div>
        ...
        <select name="status" id="">
            <option value="ADMIN" <?= isset($userCurrent) && $userCurrent->get('status') === 'ADMIN' ? 'selected' : '' ?>>Admin</option>
            <option value="MANAGER" <?= isset($userCurrent) && $userCurrent->get('status') === 'MANAGER' ? 'selected' : '' ?>>Manager</option>
            <option value="EQUIPIER" <?= isset($userCurrent) && $userCurrent->get('status') === 'EQUIPIER' ? 'selected' : '' ?>>Equipier</option>
        </select>
    </div>
    <div class="flex">
